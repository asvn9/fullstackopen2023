...
test('<CreateForm /> return the function with correct data', async () => {
  ...
  expect(createBlog.mock.calls[0][0]).toEqual({ title: 'testing a form', author: 'mr tester', url: 'www.url.fi' })

})

test('clicking view shows url, likes and the user', async () => {
  const blog = {
  ...
  expect(bloguser).toBeInTheDocument()
